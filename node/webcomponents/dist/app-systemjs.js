var sjst = (function () {
	...
	*/
	    ...
	    return (
	      ...
	      function (_super) {
	        ...
	        BaseComponent.prototype.renderInternal = function (e, index) {
	          var _this = this; //if (e) e = types.services.intercept(e);
	  ...
	  function () {
	        ...
	        return (
	          ...
	          function (_super) {
	            ...
	            function Proxy(props) {
	              return _super.call(this, app) || this; //this.app = new App(props);
	            }
	    ...
	    Processor.prototype.processElement = function (obj, index) {
	      ...
	      if (Array.isArray(obj) && obj.some(function (c) {
	        ...
	        id: Date.now()
	      }, obj], 0, obj && obj[1] && obj[1].key !== undefined ? obj[1].key : index);else if (typeof obj === "string" || !obj) return obj;
	  ...
	  function () {
	    function Transformer(settings) {
	      ...
	      this.settings.parsers[".require"] = this.settings.parsers[".import"] = function (obj, parseSettings, offset) {
	        return _this.loadModule(_this._process(obj[".import"] || obj[".require"], false, false, parseSettings, offset), parseSettings, offset);
	      };

	      this.settings.parsers[".function"] = function (obj, parseSettings, offset) {
	        return "function " + (obj[".function"] ? obj[".function"] : "") + "(" + (obj["arguments"] ? _this._process(obj["arguments"], false, true, parseSettings, offset) : "") + "){ return " + _this._process(obj["return"], true, false, parseSettings, offset) + " }";
	      };

	      this.settings.parsers[".map"] = function (obj, parseSettings, offset) {
	        return _this._process(obj[".map"], false, false, parseSettings, offset) + ".map(function(" + obj["arguments"] + ") {return " + (settings && settings.indent ? new Array(offset).join(' ') : "") + _this._process(obj["return"], true, false, parseSettings, offset) + " })";
	      };

	      this.settings.parsers[".filter"] = function (obj, parseSettings, offset) {
	        return _this._process(obj[".filter"], false, false, parseSettings, offset) + ".filter(function(" + obj["arguments"] + ") {return " + _this._process(obj["condition"], true, false, parseSettings, offset) + " })";
	      };

	      this.settings.parsers[".call"] = function (obj, parseSettings, offset) {
	        return _this._process(obj[".call"], false, false, parseSettings, offset) + ".call(" + (obj["arguments"] ? _this._process(obj["arguments"], false, true, parseSettings, offset) : "") + ")";
	      };

	      this.settings.parsers[".exec"] = function (obj, parseSettings, offset) {
	        return _this._process(obj[".exec"], true, false, parseSettings, offset) + "(" + (obj["arguments"] ? _this._process(obj["arguments"], true, true, parseSettings, offset) : "") + ")";
	      };

	      this.settings.parsers[".new"] = function (obj, parseSettings, offset) {
	        return "new " + _this._process(obj[".new"], true, false, parseSettings, offset) + "(" + (obj["arguments"] ? _this._process(obj["arguments"], true, true, parseSettings, offset) : "") + ")";
	      };
	      ...
	      this.settings.parsers[".webapp"] = function (obj, parseSettings, offset) {
	        ...
	        return "" + _this._process({
	          ".new": {
	            ".require": "@appfibre/webapp#WebApp"
	          },
	          "arguments": [obj2]
	        }, true, true, parseSettings, offset);
	      };
	    ...
	    Transformer.prototype._process = function (obj, esc, et, parseSettings, offset) {
	      ...
	      if (obj === null) output = "null";else if (Array.isArray(obj)) output = (et ? "" : "[") + this.format(obj.map(function (e, i) {
	        return _this._process(e, esc, false, parseSettings, offset + 1);
	        ...
	        if (!processed) output = (et ? "" : "{") + this.format(keys.filter(function (k) {
	          ...
	          return (_this.reservedWords.indexOf(k) > -1 || /[^a-z0-9]/i.test(k) ? "\"" + k + "\"" : k) + ":" + (_this.settings.compact ? '' : ' ') + _this._process(obj[k], esc, false, parseSettings, offset + 1);
	      ...
	      switch (this.settings.module.toLowerCase()) {
	        ...
	        case "cjs":
	          ...
	          output.code += keys.map(function (key) {
	            return "module.exports['" + key + "']" + sp + "=" + sp + _this._process(obj[key], true, false, output, 0) + ";";
	          }).join(nl);
	          if (!isDefault) output.code += nl + "module.exports['default']" + sp + "=" + sp + "{" + sp + keys.map(function (key) {
	            return key + ": " + _this._process(obj[key], true, false, output, 0);
	          }).join(nl) + " };";
	          break;

	        case "es":
	          if (isDefault) output.code += "export default" + sp + this._process(obj["default"], true, false, output, 0) + ";";else {
	            output.code += "export default" + sp + "{" + this.format(keys.map(function (key) {
	              return validkeys.indexOf(key) === -1 ? "\"" + key + "\": " + _this._process(obj[key], true, false, output, 0) : key + ":" + sp + (_this.settings.namedExports ? key : _this._process(obj[key], true, false, output, 2));
	            }), output, 1) + "};";
	            if (this.settings.namedExports && validkeys.length > 0) output.code = validkeys.map(function (key) {
	              return "export " + vr + " " + key + sp + "=" + sp + _this._process(obj[key], true, false, output, 1) + ";";
	...
	var types_1$1 = types$1.LogLevel;
	var types_2$1 = types$1.ModuleSystem;
	var types_3 = types$1.browserType;
	    ...
	    WebUI.prototype.processElement = function (element, depth, index) {
	      if (depth % 2 === 0) {
	        if (typeof element != "string" && !Array.isArray(element)) {
	          ...
	          if (element[1] && element[1].style) this.overrideStyles(element[1].style);
	  ...
	  function (_super) {
	    ...
	    function WebApp(app, context) {
	      ...
	      if ((typeof document === "undefined" ? "undefined" : _typeof(document)) === "object") {
	        ...
	        if (w && g && d) {
	          if (g.InstallTrigger !== undefined) _this.info.browser = types.browserType.FireFox;else if (
	          /*@cc_on!@*/
	        ...
	        _this.services.processor.process(ui).then(function (value) {
	          try {
	            ...
	            if ((typeof document === "undefined" ? "undefined" : _typeof(document)) === "object") {
	              ...
	              } else if (!document.body) document.body = document.createElement('BODY');
	...
	var _jst = new externals["@appfibre/core"].Transformer({
	  module: 'amd'
	});
