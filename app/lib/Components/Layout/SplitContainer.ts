...
declare interface initial {
    size?:number
    min?:number
    max?:number
};

declare interface panel extends initial {
    ratio?:any
    content: any
}

let SplitContainer : types.Component = function inject(app:types.IAppLoaded) {
    return class SplitContainer extends app.services.UI.Component {
        ...
        private distributeSpace(panels:Array<panel>, available:number):Array<panel> {
            var n = 0;
                ...
                if ((!prev.max || prev.max >= p + delta) && (!prev.min || prev.min <= p + delta) && (!next.min || next.min <= n - delta) && (!next.max || next.max >= n - delta)) {
                    this.setState({panels: this.state.panels.map((c, i) => { 
                        if (i === index || i === index + 1) {
                            if (!c.size || app.info.web && app.info.web.browser === types.browserType.Safari) {
