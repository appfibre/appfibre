...
fbt.run('ui - react', 'app', '.input.json', '.expected.html', '.react.html', (input:any, filename:string)=>{ 
    return new Promise((resolve:Function, reject:Function) => {
        ...
        var app_react = new WebApp({main:input, settings: {logLevel: appfibre.LogLevel.Trace, baseExecutionPath: filename.substr(0, filename.lastIndexOf('\\'))}, services: {UI: reactUI, logger: deferredLogger, transformer: new Transformer({module: appfibre.ModuleSystem.CommonJS})},  components: { "Tests": {"Test" : Test} }});
    ...
    fbt.run('ui - preact', 'app', '.input.json', '.expected.html', '.preact.html', (input:any, filename:string)=>{ 
        return new Promise((resolve:Function, reject:Function) => {
            ...
            var app_preact = new WebApp({main:input, settings: {logLevel: appfibre.LogLevel.Trace, baseExecutionPath: filename.substr(0, filename.lastIndexOf('\\'))}, services: {UI: preactUI, transformer: new Transformer({module: appfibre.ModuleSystem.CommonJS}), logger: deferredLogger}, components: { "Tests": {"Test" : Test} }});
