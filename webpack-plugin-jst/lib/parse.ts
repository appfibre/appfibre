...
export default (r: ref, options:ITransformPluginArgs): PluginObj<any> => ({
  visitor: {
    ObjectProperty(path:NodePath<babel.types.ObjectProperty>, state) {
      const visitor = {   
        TemplateLiteral(path:NodePath<any>):void {
            if (path.parentPath.node.type === "ArrayExpression" && path.key === 0 && path.parentPath.container) 
            {
              var elements:Array<babel.types.Expression> = [];
